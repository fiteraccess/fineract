...
public class NipSwitchAccountingConfigurationValidator {
    ...
    public void validateForUpsert(String switchId, JsonCommand command) {
        ...
        validator.reset().parameter(SWITCH_PAYABLE_GL_ACCOUNT_ID).value(switchPayableGlAccountId).notNull().longGreaterThanZero();
        validator.reset().parameter(SWITCH_FEE_GL_ACCOUNT_ID).value(switchFeeGlAccountId).notNull().longGreaterThanZero();
